...
class_table |>
  ...
  tab_source_note(
    source_note = "Note. SD is computed from the class-specific within-class variance."
  )
```
...
pairwise_order |>
  gt() |>
  tab_header(
    title = "Pairwise p-Value Order",
    subtitle = "Order used to match entered p-values to class comparisons"
  ) |>
  cols_label(
    position = "Position",
...
pairwise_display |>
  ...
  tab_header(
    ...
    subtitle = "Full diagnostic table for effect-size and p-value adjustment quantities"
  ) |>
  cols_label(
    comparison = "Comparison",
...
primary_table |>
  ...
  tab_header(
    ...
    subtitle = "Mean differences, standardized effects, confidence intervals, and BH/FDR-adjusted p-values"
  ) |>
  cols_label(
    comparison = "Comparison",
    mean_difference = "Mean difference",
